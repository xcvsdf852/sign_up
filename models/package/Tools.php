    ...
    static function sendResetPasswordMail($data){
    	$mailHTML = '
			...
			<div class="box">
			    ...
			    <br />
			    <div class="copyright">此信件由 記帳管理 系統自動發送，請勿直接回覆。</div>
			</div>
			</body>
			</html>';
		...
		$mail->FromName = "記帳管理";           //設定寄件者姓名
		$mail->Subject = "密碼重置信件 - 記帳管理";    //設定郵件標題
		$mail->Body = $mailHTML;  //設定郵件內容
		$mail->IsHTML(true);                     //設定郵件內容為HTML
		$mail->AddAddress($data['mail'], $data['nickname']); //設定收件者郵件及名稱        
		if(!$mail->Send()) {
